...
/// Remove the first `src/` path segment (either a leading `src/` or an
/// interior `/src/`), keeping the rest of the path intact.
///
/// Returns the path unchanged when it contains no `src/` segment.
fn strip_src_segment(path: &str) -> std::borrow::Cow<'_, str> {
    if let Some(rest) = path.strip_prefix("src/") {
        return std::borrow::Cow::Borrowed(rest);
    }
    if let Some(pos) = path.find("/src/") {
        // path[pos + 4..] starts with '/', so joining keeps a single separator.
        let mut stripped = String::with_capacity(path.len() - 4);
        stripped.push_str(&path[..pos]);
        stripped.push_str(&path[pos + 4..]);
        return std::borrow::Cow::Owned(stripped);
    }
    std::borrow::Cow::Borrowed(path)
}

/// Suffix path matching that also tolerates a missing `src/` segment on
/// either side.
///
/// atoms.json files generated by probe-verus < 6.9.1 could carry `code-path`
/// values that lost the `src/` segment (e.g. `crate/lemmas/foo.rs` instead of
/// `crate/src/lemmas/foo.rs`), which made them unmatchable against parser
/// paths. This helper treats such pairs as equivalent by comparing the paths
/// again after removing the first `src/` segment from each side.
pub fn paths_match_by_suffix_ignoring_src(path1: &str, path2: &str) -> bool {
    if paths_match_by_suffix(path1, path2) {
        return true;
    }
    paths_match_by_suffix(&strip_src_segment(path1), &strip_src_segment(path2))
}
...
mod tests {
    ...
    #[test]
    fn test_strip_src_segment() {
        assert_eq!(strip_src_segment("src/lib.rs"), "lib.rs");
        assert_eq!(strip_src_segment("crate/src/lib.rs"), "crate/lib.rs");
        assert_eq!(
            strip_src_segment("crate/src/lemmas/foo.rs"),
            "crate/lemmas/foo.rs"
        );
        assert_eq!(strip_src_segment("crate/lib.rs"), "crate/lib.rs");
        assert_eq!(strip_src_segment("build.rs"), "build.rs");
    }

    #[test]
    fn test_paths_match_by_suffix_ignoring_src() {
        // Plain suffix matches still work
        assert!(paths_match_by_suffix_ignoring_src(
            "/project/src/lib.rs",
            "src/lib.rs"
        ));

        // Missing src/ segment on one side (issue #33: pre-6.9.1 atoms.json)
        assert!(paths_match_by_suffix_ignoring_src(
        ...
        // Crate-root files are unaffected
        assert!(paths_match_by_suffix_ignoring_src("build.rs", "build.rs"));
        assert!(!paths_match_by_suffix_ignoring_src(
            "crate/build.rs",
            "other/build.rs"
        ));

        // Different directories still don't match after stripping src/
        assert!(!paths_match_by_suffix_ignoring_src(
            "crate/src/foo.rs",
            "crate/tests/foo.rs"
        ));
