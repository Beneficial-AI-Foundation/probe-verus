...
- **cfg-inactive functions are classified out of scope** ([#38](https://github.com/Beneficial-AI-Foundation/probe-verus/issues/38), KB P26): a function whose `#[cfg(...)]` predicate is false in the verification build (resolved default features + `verus_keep_ghost`) is not compiled and cannot be verified, so it is now `verification-status: "excluded"`, `excluded-reason: "cfg-inactive"`, `is-disabled: false` — no longer polluting the backlog. Covers `#[cfg(test)]` code, inactive features (`serde`/`group`/`rand_core`), and `#[cfg(not(verus_keep_ghost))]` fallbacks. New `cfg` (predicate) and `excluded-reason` output fields; new `cfg_eval` module resolves the feature graph and evaluates predicates conservatively (an unresolvable predicate leaves the atom as-is, never hiding real backlog). Inclusion gates (`verus_keep_ghost`, active features like `alloc`/`precomputed-tables`/`zeroize`/`digest`) are correctly kept in scope.
...
- **`is-disabled` respects any `verification-status`** ([#38](https://github.com/Beneficial-AI-Foundation/probe-verus/issues/38)): a genuinely spec-less exec function that Verus still verifies (no `requires`/`ensures`, but Verus checks its body and callee preconditions) previously kept `is-disabled: true` alongside a `verified` status, violating KB P25. The merge now sets `is-disabled: false` for any atom carrying a `verification-status`, while preserving `None` ("not analyzed") when specs were not run.
